...
def nextflow_params_parser(params_str):
    """
    Convert a nextflow params string into a dictionary.
    """
    result = {}
    ...
    while "," in params_str or "[" in params_str:
        ...
        if params_str[next_sep + 1] == "[":
            # nested, skip for brevity
            value = {}
            item_end = params_str.find("]") + 1
        else:
...
def plotly_components(charts: list, tab_class: str = "tab") -> tuple[str, str]:
    """
    Returns a single script and an optional grid-based HTML div structure for all charts.
    """
    div_blocks = []
    script_calls = []
    ...
    for chart in charts:
        ...
        # Each plot cell is inline-block, width is respected from the figure itself
        div_blocks.append(
            f'<div id="{div_id}" class="{tab_class}-plot-cell" '
            f'style="display:inline-block; vertical-align:top;"></div>'
        )
    ...
    div_grid = f"""
    <div class="{tab_class}-plot-grid" style="
        display: flex;
        flex-wrap: wrap;
        gap: 10px;
    ...
    # One script to render all plots
    script = f"""
    <script>
...
<head>
    <meta charset="UTF-8">
    <title>{{ additional_info.sample_name }} Report</title>
    ...
    <div class="tab-content mt-3" id="reportTabsContent">
        ...
        <div class="tab-pane fade show active" id="details" role="tabpanel">
            ...
            <ul class="list-group mb-3">
                ...
                <li class="list-group-item bg-light"></li> <!-- spacer -->
        ...
        <!-- Plot tabs -->
        {% for tab in plot_items %}
            <div class="tab-pane fade" id="tab-{{ loop.index }}" role="tabpanel">
                {{ tab.plot | safe }}
            </div>
        {% endfor %}
