...
class VCF_file:
    ...
    def read_vcf(self, path):
        ...
        for i, fmt in enumerate(formats):
            df[fmt] = df.Sample1.apply(lambda x: float((x.split(':')[i] if (x.split(':')[i]) else 0)))
        return df
    ...
    def write(self, path):
        with open(path,'w') as new_vcf:
            ...
            writeable_vcf = writeable_vcf[['#CHROM', 'POS','ID', 'REF', 'ALT','QUAL', 'FILTER', 'INFO']]
    ...
    def filter(self, min_dir_ratio = 0.001,min_dir_count = 5, min_dqp= 500 ,min_vaf = 0.003, min_dir_ratio_ratio= 0.1):
