...
def get_roi_pileup_df(df: pd.DataFrame, distance: int = 100) -> List[chromosomal_region]:
    """
...
def pileup_to_df(pileup_path:str) -> pd.DataFrame:
    ...
    with open(pileup_path, 'r') as pu:
        ...
        df1 = df1.join(df1.apply(_parse_pileup, axis=1))
        df1['ACCURACY'] = df1.apply(compute_accuracy, insertions=True, deletions=True, axis=1)
        df1['Q'] = df1.ACCURACY.apply(acc_to_Q)

        return df1


def compute_accuracy(row: pd.Series, insertions=True, deletions=True, by_strand=False) -> float:
    ...
    if not by_strand:
        ...
        cov = row[['A','T','C','G','N','a','t','c','g','n','fINS','fDEL','rINS','rDEL']].sum()
        if not insertions: 
...
def plot_compare_accuracy(roi: List[chromosomal_region], df_list: List[pd.DataFrame], my_title='untitled'):
    """
    ...
    for region in roi:
        ...
        p_freq.line("POS", "Q", source=df2, line_width=2,color = 'orange', legend_label = "Consensus")
        plots.append(p_freq)
...
def make_qscore_scatter(df1,df2):
    ...
    df_merge = pd.merge(pre, post, how='left', left_on=['CHROM','POS'], right_on=['CHROM','POS'])
...
if __name__ =="__main__":
    ...
    # pileup_split = '/media/dami/a2bc89fb-be6b-4e23-912a-0c7137cd69ad/results/Cyclomics/000010_dev/PostQC/MPileupSplit/splibams_merged.pileup'
    # pileup_cons = '/media/dami/a2bc89fb-be6b-4e23-912a-0c7137cd69ad/results/Cyclomics/000010_dev/PostQC/MPileupConsensus/FAS37222.pileup'
